const audio = new Audio('./res/loon.wav')
...
document.getElementById('loon-sound').addEventListener("click", () => {
    audio.play();
    console.log('~loon sounds~')
})


const getMeetings = (resource, callback) => {
...
// --- Add scraping of WP here --- //

// --- Retreive meetings and process them --- //

getMeetings('./meetings/events.json', (err, data) => {
    let headerAry = []
    let meetingsAry = []
    if(err){
        console.log(err)
